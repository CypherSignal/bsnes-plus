...
void SymbolMap::finishUpdates() {
  ...
  if (addressToSourceLineMappings.size() > 0) {
    isValid = false;
    sourceLines.reset();
    addressToSourceLineMappings.sort();
...
void SymbolMap::loadFromString(const string &file) {
  ...
  for (int i = 0; i < rows.size(); i++) {
    ...
    if (row[0] == '[') {
      ...
      else if (row == "[rom checksum]") { section = SECTION_CHECKSUM; }
...
{  
  ...
  f.print("; wla-style symbol file generated by bsnes-plus\n");
  f.print(";\n");
  ...
  for (uint32_t i = 0; i < labels.size(); i++) {
    ...
    f.print(
      hex<2, '0'>(label.address >> 16), ":", 
      hex<4, '0'>(label.address & 0xFFFF), " ",
      label.text, "\n");
  }
  ...
  for (uint32_t i = 0; i < comments.size(); i++) {
    ...
    f.print(
      hex<2, '0'>(comment.address >> 16), ":",
      hex<4, '0'>(comment.address & 0xFFFF), " ",
      comment.text, "\n");
  }
  ...
  for (uint32_t i = 0; i < addressToSourceLineMappings.size(); i++) {
    const auto& addrToLine = addressToSourceLineMappings[i];
    f.print(
      hex<2, '0'>(addrToLine.address >> 16), ":",
      hex<4, '0'>(addrToLine.address & 0xFFFF), " ",
      hex<4, '0'>(addrToLine.file), ":",
      hex<8, '0'>(addrToLine.line), "\n");
  }
