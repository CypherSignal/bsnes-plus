...
void SFXDebugger::op_step() {
  if (pc_valid) {
    ...
    if(debugger.step_sfx &&
      ...
      debugger.breakpoint_test(Debugger::BreakpointMemoryBus::SFXBus, Debugger::Breakpoint::Mode::Exec, opcode_pc, 0x00);
    }
...
uint8 SFXDebugger::rombuffer_read() {
  ...
  debugger.breakpoint_test(Debugger::BreakpointMemoryBus::SFXBus, Debugger::Breakpoint::Mode::Read, fulladdr, data);
...
void SFXDebugger::rambuffer_write(uint16 addr, uint8 data) {
  ...
  debugger.breakpoint_test(Debugger::BreakpointMemoryBus::SFXBus, Debugger::Breakpoint::Mode::Write, fulladdr, data);
